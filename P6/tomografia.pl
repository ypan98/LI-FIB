...
myLength(N, L):- length(L, N).
matrixByRows(L,NumCols,MatrixByRows):- length(L, Size), NumRows is Size/NumCols, length(MatrixByRows, NumRows),
																			 maplist(myLength(NumCols), MatrixByRows), append(MatrixByRows, L).

declareConstraints(MatrixByRows, MatrixByCols, RowSums, ColSums):- sumConstraint(MatrixByRows, RowSums), sumConstraint(MatrixByCols, ColSums).

sumConstraint([], _).
sumConstraint([Vec|Mat], [S|Sums]):- sum(Vec, #=, S), sumConstraint(Mat, Sums).
