...
incompatibleWorkers(W1,W2):-score(W1, S1), score(W2, S2), Sum is S1 + S2, maxScore(Max), Sum > Max. %% SHOULD BE COMPLETED!!!!
complexWorkers(W1,W2):-score(W1, S1), score(W2, S2), Sum is S1 + S2, complexThreshold(Max), Sum >= Max.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1.- Declare SAT variables to be used
% wt(W,T) means "worker W is in team T"
satVariable( wt(W,T) ):- worker(W), team(T).
satVariable( ct(T) ):- team(T).
...
writeClauses(MaxComplexTeams):-
    ...
    teamsMoreThanMin,
    incompatibility,
    comlexTeamImplication,
    complexTeamUpperBound(MaxComplexTeams),
    true,!.
...
teamsMoreThanMin:- team(T), findall(wt(W, T), worker(W), Lits), minSize(Min), atLeast(Min, Lits), fail.
teamsMoreThanMin.

teamsLessThanMax:- team(T), findall(wt(W, T), worker(W), Lits), maxSize(Max), atMost(Max, Lits), fail.
teamsLessThanMax.

incompatibility:- worker(W1), worker(W2), W1 \= W2, incompatibleWorkers(W1, W2), team(T), writeClause([-wt(W1, T), -wt(W2, T)]), fail.
incompatibility.

comlexTeamImplication:- worker(W1), worker(W2), W1 \= W2, complexWorkers(W1, W2), team(T), writeClause([-wt(W1, T), -wt(W2, T), ct(T)]), fail.
comlexTeamImplication.

complexTeamUpperBound(MaxComplexTeams):- findall(ct(T), team(T), Lits), atMost(MaxComplexTeams, Lits), fail.
complexTeamUpperBound(_).
...
costOfThisSolution(M,Cost):- findall(T, member(ct(T), M), Lits), sort(Lits, L), length(L, Cost).
