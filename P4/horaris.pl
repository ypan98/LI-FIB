...
writeClauses(MaxNumProf):-
		...
		eachCourseOneRoom,
		eachCourseOnceADay,
		noCoursesSameTimeSameProf,
		noCoursesSameTimeSameRoom,
		noCoursesSameLevelSameTime,
		cdImplication,
		courseHoursConstraint,
		pImplication,
		atMostMaxProf(MaxNumProf),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachCourseOnePof:- course(C), courseProfessors(C, LP), findall(cp(C,P), member(P, LP), Lits), exactly(1, Lits), fail.
eachCourseOnePof.

eachCourseOneRoom:- course(C), courseRooms(C, LR), findall(cr(C,R), member(R, LR), Lits), exactly(1, Lits), fail.
eachCourseOneRoom.

eachCourseOnceADay:- course(C), day(D), findall(cdh(C,D,H), hour(H), Lits), atMost(1, Lits), fail.
eachCourseOnceADay.

noCoursesSameTimeSameProf:- course(C1), course(C2), C1 \= C2, courseProfessors(C1, LP1), courseProfessors(C2, LP2), day(D), hour(H),
														member(P, LP1), member(P, LP2), writeClause([-cdh(C1, D, H), -cdh(C2, D, H), -cp(C1, P), -cp(C2, P)]), fail.
noCoursesSameTimeSameProf.

noCoursesSameTimeSameRoom:- course(C1), course(C2), C1 \= C2, courseRooms(C1, LR1), courseRooms(C2, LR2), day(D), hour(H),
														member(R, LR1), member(R, LR2), writeClause([-cdh(C1, D, H), -cdh(C2, D, H), -cr(C1, R), -cr(C2, R)]), fail.
noCoursesSameTimeSameRoom.

noCoursesSameLevelSameTime:- course(C1), course(C2), C1 \= C2, courseYear(C1, Y), courseYear(C2, Y), day(D), hour(H),
														writeClause([-cdh(C1, D, H), -cdh(C2, D, H)]), fail.
noCoursesSameLevelSameTime.

%cdImplication:- course(C), day(D), hour(H), writeClause([-cdh(C,D,H), cd(C,D)]), fail.
cdImplication:- course(C), day(D), findall(cdh(C,D,H), hour(H), Lits), expressOr(cd(C,D), Lits), fail.
cdImplication.

courseHoursConstraint:- course(C), courseHours(C, CH), findall(cd(C,D), day(D), Lits), exactly(CH, Lits), fail.
courseHoursConstraint.

pImplication:- course(C), professor(P), writeClause([-cp(C,P), p(P)]), fail.
pImplication.

atMostMaxProf(MaxNumProf):- findall(p(P), professor(P), Lits), atMost(MaxNumProf, Lits), fail.
atMostMaxProf(_).
...
costOfThisSolution(M,Cost):- findall(P, member(p(P), M), Lits), sort(Lits,L), length(L, Cost).
