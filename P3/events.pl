...
eventsWithModerator:- event(E), eventModerators(E, LM), findall(em(E,M), member(M, LM), Lits), exactly(1, Lits), fail.
eventsWithModerator.

eventWithDay:- event(E), eventDays(E, LD), findall(ed(E,D), member(D, LD), Lits), exactly(1, Lits), fail.
eventWithDay.

eventsPerDay:- day(D), findall(ed(E,D), event(E), Lits), maxEventsPerDay(Max), atMost(Max, Lits), fail.
eventsPerDay.

mdImplication:- event(E), eventModerators(E, LM), member(M, LM), eventDays(E, LD), member(D, LD), writeClause([-ed(E,D), -em(E,M), md(M,D)]), fail.
mdImplication.

moderatorMaxDay:- moderator(M), findall(md(M,D), day(D), Lits), maxDaysPerModerator(Max), atMost(Max, Lits), fail.
moderatorMaxDay.
