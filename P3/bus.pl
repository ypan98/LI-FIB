...
satVariable( td(C1, C2, D) ):- trip(C1-C2), day(D).


writeClauses:-
    everyDayEachCityOneBus,
    eachBusEachDayOneCity,
    busCantStaySameCity,
    noMoreThanMaxDist,
    tdRelation,
    %eachDayNumBusTrips,
    eachCityAtLeastOneTripPerWeek,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

everyDayEachCityOneBus:- city(C), day(D), findall(bdc(B,D,C), bus(B), Lits), exactly(1, Lits), fail.
everyDayEachCityOneBus.

eachBusEachDayOneCity:- bus(B), day(D), findall(bdc(B, D, C), city(C), Lits), exactly(1, Lits), fail.
eachBusEachDayOneCity.

busCantStaySameCity:- consecutiveDays(D1, D2), bus(B), city(C), writeClause([-bdc(B,D1,C), -bdc(B,D2,C)]), fail.
busCantStaySameCity.

noMoreThanMaxDist:- consecutiveDays(D1, D2, D3), bus(B), trip(C1-C2), trip(C2-C3), maxDist(Max), dist(C1, C2, D12), dist(C2, C3, D23),
                    DTotal is D12+D23, DTotal > Max, writeClause([-bdc(B, D1, C1), -bdc(B, D2, C2), -bdc(B, D3, C3)]), fail.
noMoreThanMaxDist.

%bcd(B,D1,C1) ^ travel(C1,C2,D1) => bcd(B,D2,C2)
tdRelation:- notLastDay(D1), consecutiveDays(D1, D2), bus(B), trip(C1-C2), writeClause([-bdc(B,D1,C1), bdc(B,D2,C2), -td(C1,C2,D1)]), fail.
tdRelation.

%eachDayNumBusTrips:- notLastDay(D), findall(td(C1, C2, D), trip(C1-C2), Lits), numBuses(B), exactly(B, Lits), fail.
%eachDayNumBusTrips.

eachCityAtLeastOneTripPerWeek:- trip(C1-C2), findall(td(C1, C2, D), notLastDay(D), Lits), atLeast(1, Lits), fail.
eachCityAtLeastOneTripPerWeek.
